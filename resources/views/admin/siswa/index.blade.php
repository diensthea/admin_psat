    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between space-y-4 md:space-y-0">
        <div>
            <h1 class="text-2xl font-extrabold text-blue-950 flex items-center">
                <i class="fa-solid fa-user-graduate mr-2 text-blue-900"></i> Data Siswa (Murid)
            </h1>
            <p class="text-sm text-gray-500">Daftar siswa peserta ujian yang dapat menandatangani daftar hadir secara digital.</p>
        </div>
        ...
        <div class="overflow-x-auto">
            @if($siswas->isEmpty())
                <div class="text-center py-12 text-gray-400">
                    <i class="fa-solid fa-user-graduate text-4xl mb-2"></i>
                    <p class="text-sm">Belum ada data siswa. Silakan tambah secara manual atau import via CSV.</p>
                </div>
            @else
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50 text-xs font-bold text-gray-500 uppercase tracking-wider">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left">NIS (Username)</th>
                        ...
                        @foreach($siswas as $index => $siswa)
                            <tr class="hover:bg-gray-50 transition">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-center space-x-2">
                                    <button onclick="openEditModal('{{ $siswa->id }}', '{{ $siswa->nis }}', '{{ $siswa->nama }}', '{{ $siswa->user->email ?? '' }}')" 
                                        class="bg-yellow-50 text-yellow-700 hover:bg-yellow-100 p-2 rounded-lg text-xs font-bold transition shadow-sm" title="Edit Siswa">
                                        <i class="fa-solid fa-pen-to-square"></i> Edit
                                    </button>
                                    
                                    <form action="{{ route('admin.siswa.reset-password', $siswa->id) }}" method="POST" class="inline">
                                        @csrf
                                        <button type="submit" onclick="return confirm('Apakah Anda yakin ingin mereset password siswa ini ke default (NIS)?')"
                                            class="bg-blue-50 text-blue-700 hover:bg-blue-100 p-2 rounded-lg text-xs font-bold transition shadow-sm" title="Reset Password ke NIS">
                                    ...
                                    <form action="{{ route('admin.siswa.destroy', $siswa->id) }}" method="POST" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" onclick="return confirm('Menghapus data Siswa juga akan menghapus akun terkait. Lanjutkan?')"
                                            class="bg-red-50 text-red-600 hover:bg-red-100 p-2 rounded-lg text-xs font-bold transition shadow-sm" title="Hapus Siswa">
    ...
    <div class="flex items-center justify-center min-h-screen px-4">
        ...
        <div class="bg-white rounded-xl overflow-hidden shadow-xl transform transition-all max-w-lg w-full relative z-10 border border-gray-150">
            <div class="bg-blue-900 px-6 py-4 text-white flex justify-between items-center">
                <h3 class="text-lg font-bold"><i class="fa-solid fa-user-plus mr-2 text-yellow-400"></i> Tambah Data Siswa</h3>
            ...
            <form action="{{ route('admin.siswa.store') }}" method="POST">
                ...
                <div class="p-6 space-y-4">
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-1">NIS (Sekaligus Username Login)</label>
                        <input type="text" name="nis" required class="w-full border border-gray-300 rounded-lg p-2.5 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <input type="email" name="email" class="w-full border border-gray-300 rounded-lg p-2.5 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm" placeholder="siswa@psat.com">
                    </div>
                    <div>
    ...
    <div class="flex items-center justify-center min-h-screen px-4">
        ...
        <div class="bg-white rounded-xl overflow-hidden shadow-xl transform transition-all max-w-lg w-full relative z-10 border border-gray-150">
            <div class="bg-yellow-600 px-6 py-4 text-white flex justify-between items-center">
                <h3 class="text-lg font-bold"><i class="fa-solid fa-user-pen mr-2"></i> Edit Data Siswa</h3>
            ...
            <form id="formEdit" method="POST">
                ...
                <div class="p-6 space-y-4">
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-1">NIS (Sekaligus Username Login)</label>
                        <input type="text" id="edit_nis" name="nis" required class="w-full border border-gray-300 rounded-lg p-2.5 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-1">Nama Lengkap Siswa</label>
    ...
    <div class="flex items-center justify-center min-h-screen px-4">
        ...
        <div class="bg-white rounded-xl overflow-hidden shadow-xl transform transition-all max-w-lg w-full relative z-10 border border-gray-150">
            <div class="bg-emerald-600 px-6 py-4 text-white flex justify-between items-center">
                <h3 class="text-lg font-bold"><i class="fa-solid fa-file-excel mr-2 text-yellow-300"></i> Import Data Siswa via CSV</h3>
            ...
            <form action="{{ route('admin.siswa.import') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="p-6 space-y-4">
                    <div class="bg-blue-50 p-4 rounded-lg border border-blue-100 text-xs text-blue-900 space-y-2">
                        ...
                        <p>File wajib berupa file CSV dengan kriteria header kolom:</p>
                        <p class="font-mono bg-blue-100 p-1.5 rounded text-center">nis,nama,email,password</p>
                        <ul class="list-disc list-inside space-y-1">
                            <li><span class="font-bold">nis</span>: Nomor Induk Siswa (Harus unik)</li>
                            <li><span class="font-bold">nama</span>: Nama lengkap siswa</li>
                            <li><span class="font-bold">email</span>: (opsional) email siswa</li>
                            <li><span class="font-bold">password</span>: (opsional) password akun. Jika kosong, password default diset sama dengan NIS.</li>
                        </ul>
                    </div>
                    <div>
                ...
                <div class="px-6 py-4 bg-gray-50 border-t flex justify-end space-x-3">
                    ...
                    <button type="submit" class="bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded-lg text-sm font-bold transition shadow">Mulai Import</button>
                </div>
            </form>
        </div>
    </div>
...
<script>
    ...
    function openEditModal(id, nis, nama, email) {
        document.getElementById('edit_nis').value = nis;
        document.getElementById('edit_nama').value = nama;
        document.getElementById('edit_email').value = email;
        
        let actionUrl = "{{ route('admin.siswa.update', ':id') }}";

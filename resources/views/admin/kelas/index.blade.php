    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between space-y-4 md:space-y-0">
        <div>
            <h1 class="text-2xl font-extrabold text-blue-950 flex items-center">
                <i class="fa-solid fa-school mr-2 text-blue-900"></i> Data Kelas
            </h1>
            <p class="text-sm text-gray-500">Daftar kelas di sekolah untuk pemetaan/penempatan siswa peserta ujian.</p>
        </div>
        ...
        <div class="overflow-x-auto">
            @if($kelases->isEmpty())
                <div class="text-center py-12 text-gray-400">
                    ...
                    <p class="text-sm">Belum ada data kelas. Silakan tambah secara manual atau import via CSV.</p>
                </div>
            @else
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50 text-xs font-bold text-gray-500 uppercase tracking-wider">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-center">Jumlah Siswa Terdaftar</th>
                        ...
                        @foreach($kelases as $index => $kelas)
                            <tr class="hover:bg-gray-50 transition">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <span class="px-3 py-1 bg-blue-55 bg-blue-105 text-blue-900 text-xs font-extrabold rounded-full">
                                        {{ $kelas->siswas_count }} Siswa
                                    </span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-center space-x-2">
                                    <button onclick="openEditModal('{{ $kelas->id }}', '{{ $kelas->nama_kelas }}')" 
                                    ...
                                    <form action="{{ route('admin.kelas.destroy', $kelas->id) }}" method="POST" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" onclick="return confirm('Menghapus kelas tidak akan menghapus data siswa, namun mengeluarkan siswa dari kelas tersebut. Lanjutkan?')"
    ...
    <div class="flex items-center justify-center min-h-screen px-4">
        ...
        <div class="bg-white rounded-xl overflow-hidden shadow-xl transform transition-all max-w-md w-full relative z-10 border border-gray-150">
            <div class="bg-blue-900 px-6 py-4 text-white flex justify-between items-center">
                <h3 class="text-lg font-bold"><i class="fa-solid fa-plus-circle mr-2 text-yellow-400"></i> Buat Kelas Baru</h3>
            ...
            <form action="{{ route('admin.kelas.store') }}" method="POST">
                ...
                <div class="p-6 space-y-4">
                    <div>
                        ...
                        <input type="text" name="nama_kelas" placeholder="Contoh: XII mipa 1, XI IPS 2" required class="w-full border border-gray-300 rounded-lg p-2.5 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                    </div>
                </div>
                <div class="px-6 py-4 bg-gray-50 border-t flex justify-end space-x-3">
                    ...
                    <button type="submit" class="bg-blue-900 hover:bg-blue-800 text-white px-4 py-2 rounded-lg text-sm font-bold transition shadow">Buat Kelas</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div class="flex items-center justify-center min-h-screen px-4">
        ...
        <div class="bg-white rounded-xl overflow-hidden shadow-xl transform transition-all max-w-md w-full relative z-10 border border-gray-150">
            <div class="bg-yellow-600 px-6 py-4 text-white flex justify-between items-center">
                <h3 class="text-lg font-bold"><i class="fa-solid fa-pen-to-square mr-2"></i> Edit Nama Kelas</h3>
            ...
            <form id="formEdit" method="POST">
                ...
                <div class="p-6 space-y-4">
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-1">Nama Kelas</label>
                        <input type="text" id="edit_nama_kelas" name="nama_kelas" required class="w-full border border-gray-300 rounded-lg p-2.5 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                    </div>
                </div>
                <div class="px-6 py-4 bg-gray-50 border-t flex justify-end space-x-3">
                    ...
                    <button type="submit" class="bg-yellow-650 bg-yellow-605 bg-yellow-600 hover:bg-yellow-700 text-white px-4 py-2 rounded-lg text-sm font-bold transition shadow">Simpan</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div class="flex items-center justify-center min-h-screen px-4">
        ...
        <div class="bg-white rounded-xl overflow-hidden shadow-xl transform transition-all max-w-md w-full relative z-10 border border-gray-150">
            <div class="bg-emerald-600 px-6 py-4 text-white flex justify-between items-center">
                <h3 class="text-lg font-bold"><i class="fa-solid fa-file-excel mr-2 text-yellow-300"></i> Import Kelas via CSV</h3>
            ...
            <form action="{{ route('admin.kelas.import') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="p-6 space-y-4">
                    <div class="bg-blue-50 p-4 rounded-lg border border-blue-100 text-xs text-blue-900 space-y-2">
                        ...
                        <p>Format CSV sangat sederhana dengan header tunggal:</p>
                        <p class="font-mono bg-blue-100 p-1 rounded text-center">nama_kelas</p>
                        <ul class="list-disc list-inside">
                            <li>Setiap baris berisi satu nama kelas.</li>
                            <li>Toleran dengan duplikasi otomatis diabaikan.</li>
                        </ul>
                    </div>
                    <div>
...
<script>
    ...
    function openEditModal(id, nama_kelas) {
        ...
        let actionUrl = "{{ route('admin.kelas.update', ':id') }}";
